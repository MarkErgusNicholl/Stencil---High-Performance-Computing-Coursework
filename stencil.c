...
void stencil(const int nx, const int ny, float * restrict image, float * restrict tmp_image) {
  ...
  tmp_image[0] = image[0] * 0.6 + (image[1] + image[0+ny]) * 0.1;

  // Case first column last row
  tmp_image[ny-1] = image[ny-1] * 0.6 + (image[ny-2] + image[ny-1+ny]) * 0.1;

  // Case last column first row
  tmp_image[ny*(nx-1)] = image[ny*(nx-1)] * 0.6 + (image[ny*(nx-1)+1] + image[ny*(nx-1)-ny]) * 0.1;
